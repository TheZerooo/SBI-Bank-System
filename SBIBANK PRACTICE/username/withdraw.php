...
if (isset($_POST['withdraw'])) {
    ...
    if ($currentBalance >= $amount && $amount > 0) {
        ...
        $tstatement="Debited ".$amount;
        $tid=rand(10000,99999);
        $query = "INSERT INTO ledger (accountno, ttype, tamount, avalbalance, ttime,tstatement,tid) 
                  VALUES ('$account','Debit','$amount','$newBalance', NOW(),'$tstatement','$tid')";
        mysqli_query($db, $query);

         $query1="update useraccount set
            balance='$newBalance' where accountno='$account'";
            mysqli_query($db,$query1);
    
        $message = "🙄Withdraw successful! New Balance: ₹" . number_format($newBalance, 2);
    } else {
