...
if(isset($_SESSION["accountNumber"])){
  ...
  $sql=" SELECT * from ledger where  accountno='$_SESSION[accountNumber]'";
  ...
  $output=mysqli_query($db,$sql);
      ...
      if(mysqli_num_rows($output) > 0){
        ...
        // $data=mysqli_fetch_assoc($output);
              ...
              echo "<td>" . htmlspecialchars(ucfirst($row['ttype'])) . "</td>";
              echo "<td>Rs " . number_format($row['tamount']) . "</td>";
              echo "<td>". htmlspecialchars($row['avalbalance'])."</td>"; // example action button
              echo "</tr>";
          }
       else {
        echo "<tr><td colspan='4'>No record found for this account number.</td></tr>";
    }
     } else {
